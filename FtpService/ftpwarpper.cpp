...
int FtpWarpper::FtpReply(FtpSession &session, int code, std::string content)
{
    ...
    {
        return SocketSource::TcpSend(session.GetControlSocket(), (unsigned char *)PackedContent.c_str(),
                                     PackedContent.size());
    }
}
...
/* IPC operation */
int FtpWarpper::IPC_RecvInstruction(int __sockfrom, FtpInstruction &__instruction)
{
...
int FtpWarpper::IPC_SendInstruction(int __sockto, FtpInstruction &__instruction)
{
    unsigned char *pInstruction = (unsigned char *)&__instruction;
...
int FtpWarpper::PASV_FtpTransferStandby(FtpSession &session, FtpInstruction &__instruction)
{
    int ListenSock;
    ...
    do{
        ...
        ReplyContent = "Entering PASV mode (" +
            std::to_string(ipv4[0]) + "," +
            std::to_string(ipv4[1]) + "," +
            std::to_string(ipv4[2]) + "," +
        ...
        __instruction.setInsContent((char *)ReplyContent.c_str(), ReplyContent.size());
...
int FtpWarpper::PORT_FtpTransferStandby(FtpSession &session, FtpInstruction &__instruction)
{
    ...
    std::sscanf(RequsetContent.c_str(), "%d,%d,%d,%d,%d,%d",
           &p_ipaddress[0], &p_ipaddress[1], &p_ipaddress[2],
            &p_ipaddress[3], &p_port[0], &p_port[1]);
...
int FtpWarpper::WORK_FtpTrySendCommand(FtpSession &session, FtpInstruction &__instruction)
{
    int result = 0;
    ...
    std::cout << strerror(SocketSource::CheckSockError(session.GetTransferSocket())) << std::endl;
    ...
    {
        __instruction.setInsExecFlag(true);
        __instruction.setInsContent("Transfer complete.", strlen("Transfer complete."));
...
int FtpWarpper::WORK_FtpTryContact(FtpSession &session, FtpInstruction &__instruction)
{
    int result;
    ...
    std::cout << __FUNCTION__ << " inet_ntoa(ip_addr) = " << inet_ntoa(ip_addr) << std::endl;
    std::cout << __FUNCTION__ << " session.GetTransferPort() = " << ntohs(session.GetTransferPort()) << std::endl;

    switch(session.GetTransferMode())
    {
        case PASV_MODE_ENABLE:
        {
            result = SocketSource::TcpAccept(session.PASVListenSock, (struct sockaddr *)&remote, 3 * 1000);
        }
        break;

        case PORT_MODE_ENABLE:
        {
            result = SocketSource::TcpConnect(inet_ntoa(ip_addr), session.GetTransferPort(), 3 * 1000);
        }
        break;
    }
...
int FtpWarpper::WORK_FtpTryFileDownload(FtpSession &session, FtpInstruction &__instruction)
{
    int result;
    int nbytes;
    int totalBytes = 0;
     ...
     do{
         ...
         do {
                 if(fileStat.st_size == totalBytes)
                 {
                     ...
                     ReplyContent = "Transfer complete. Total " + std::to_string(totalBytes) + " bytes.";
                     break;
                 }
                ...
                result = SocketSource::IOMonitor(&TransferSocket, 1, 100, WRITEFDS_TYPE, fds);
                if(result > 0)
                {
                    if(FD_ISSET(TransferSocket, &fds))
                    {
                        ...
                        nbytes = sendfile(session.GetTransferSocket(), fileSocket, NULL, nbytes);
                        if(nbytes > 0)
                        {
                            totalBytes += nbytes;
                            continue;
...
int FtpWarpper::WORK_FtpTryFileUpload(FtpSession &session, FtpInstruction &__instruction)
{
    int result;
    int fileSocket;
    std::string __filepath;
    ...
    cc.convert((char *)__filepath.c_str(), strlen(__filepath.c_str()),out,CONVEROUTLEN);
        ...
        do{
                ...
                if(result > 0)
                {
                    if(FD_ISSET(TransferSocket, &fds))
                    {
                        ...
                        result = sendfile(fileSocket, session.GetTransferSocket(), NULL, nbytes);
                        if(result > 0)
                        {
...
int FtpWarpper::__FTP_CWD(FtpSession &session, char *context)
{
    ...
    cc.convert((char *)__ftpPath.c_str(), strlen(__ftpPath.c_str()),out,CONVEROUTLEN);
...
int FtpWarpper::__FTP_LIST(FtpSession &session, char *context)
{
    ...
    IPC_SendInstruction(session.GetIPCContorlFD(), instruction);
    FtpReply(session, FTP_DATACONN, std::string("Opening ASCII mode data connection for /bin/ls"));
...
void FtpWarpper::SignalHandler(int __signalNo, siginfo_t* __signalInfo,void* __content)
{
...
int FtpWarpper::ServiceStart(FtpSession &session)
{
    ...
    else if(pid > 0)
    {
        ...
        std::cout << "PID : " << getpid() << " FTP-CONTORL Processer exit." << std::endl;
    }
...
int FtpWarpper::FTPControlHandler(FtpSession &session)
{
    ...
    FtpReply(session, FTP_GREET, "Environment: Linux system. Used UNIX BSD Socket. (FtpServer ver. 0.1)");
    ...
    do{
        ...
        memset(buffer, 0, sizeof(buffer));
        nbytes = SocketSource::IOMonitor(SocketList, sizeof(SocketList)/sizeof(int), 100, READFDS_TYPE, fds);
        if(nbytes == 0)
            continue;
        else if(nbytes > 0)
        {
            ...
            {
                nbytes = SocketSource::TcpReadOneLine(session.GetControlSocket(), buffer, sizeof(buffer));
                if(nbytes <= 0)
                {
...
int FtpWarpper::IPC_FTPControlHandler(FtpSession &session, FtpInstruction &__instruction)
{
    int ResponseCode;
    std::string ReplyContent;
