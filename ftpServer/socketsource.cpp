...
int SocketSource::IOMonitor(int *SockQueue, int QueueSize, int timeout, int option, fd_set &optionfds)
{
...
int SocketSource::TcpServerCreate(const char *lhost, unsigned short lport)
{
    ...
    assert(0 == bind(listenfd, (struct sockaddr *)&ServAddr, sizeof(struct sockaddr)));
...
int SocketSource::TcpAccept(int ServSocket, struct sockaddr *client, int timeout)
{
    fd_set readfds;
    struct timeval select_timeout;
    int result;
...
int SocketSource::TcpConnect(const char *host, unsigned short port, int timeout)
{
    if(host == NULL)
        return -1;
    ...
    do{
        ...
        if( 0 == connect(sockfd, (struct sockaddr *)&remote, sizeof(struct sockaddr)) )
        {
...
int SocketSource::UdpRecv(int sockfd, sockaddr *from, unsigned char *data, int nbytes)
{
    int rbytes;
    int total_bytes = 0;
    socklen_t socklen = sizeof(struct sockaddr);
    ...
    do{
        ...
        rbytes = recvfrom(sockfd, (char *)pData, nbytes - total_bytes, 0, from, &socklen);
        if(rbytes == 0)
        {
            return 0;
        }
...
int SocketSource::UdpSend(int sockfd, sockaddr *dest, unsigned char *data, int nbytes)
{
    int wbytes;
    int total_bytes = 0;
    ...
    do{
        ...
        wbytes = sendto(sockfd, (char *)pData, nbytes - total_bytes, 0, dest, socklen);
        if(wbytes == -1 && errno == EINTR)
            continue;
